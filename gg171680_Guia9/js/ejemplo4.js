...
buttonMenu.onclick = function() {
    //Verificando que exista el conetendor de la pagina
...
buttonTitulo.onclick = function(){
    ...
    const contenedor = document.querySelector("#id DivPage");
    ...
    if(contenedor) {
        if (menu.length > 0) {
            ...
            if (titulo != "" && titulo != null) {
                ...
                alert(
                    "No se ha registrado ninun titulo, por favor ingrese informacion"
                );
            }
        }   else {
        ...
        alert("Primero debe agregar un conetenedor de pagina");
    }
};

buttonParrafo.onclick = function() {
    //Verificando que exista el contendor de la pagina
    const contenedor = document.querySelector("#idDivPage");

    //Verificando que exita el meni
    const menu = document.querySelector("#idDivPage > header");
    ...
    if (contenedor) {
        if(menu.length > 0) {
            ...
            alert("Primero debe agregar un menu primero");
        }
    }   else {
        alert("Primero debe agregar un contenedor de pagina");
    }
